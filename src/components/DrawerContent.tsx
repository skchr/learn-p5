...
export default function DrawerContent({ onClose }: DrawerContentProps) {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.surface }]}>
      <View
        style={[styles.header, { paddingTop: insets.top, backgroundColor: derivedColors.primary }]}
      >
      ...
      <View style={styles.navList}>
        {navItems.map((item) => (
          <Pressable
            ...
            style={[
              styles.navItem,
              isActive(item.href) && { backgroundColor: derivedColors.primary + "1A" },
            ]}
          ...
          >
            <View style={styles.iconContainer}>
              <MaterialCommunityIcons
                name={item.icon}
                size={24}
                color={isActive(item.href) ? derivedColors.primary : colors.textSecondary}
              />
            </View>
            <Text style={[
              styles.navLabel,
              isActive(item.href)
                ? { color: derivedColors.primary, fontWeight: "600" }
